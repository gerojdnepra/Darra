...
const defaultBackendWsUrl = "ws://localhost:3001/ws";
...
const terminalCopy: Record<InterfaceLanguage, CopyBlock> = {
  en: {
    ...
    workspace: "Live desktop workspace",
    ...
    emptyDescription:
      "Add terminal widgets, bind the tab to a symbol, and keep a separate working layout for each scenario. Quotes, order book, chart, alerts, and window controls are now driven by the real backend feed.",
    ...
    moduleWindowsTitle: "Module windows",
...
export function DarraTerminalShell() {
  ...
  const recentAlerts = frame?.alerts?.slice(0, 6) ?? [];
