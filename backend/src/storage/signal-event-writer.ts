...
export class SignalEventWriter {
  ...
  private duplicateSignalCount = 0;
  private lastDuplicateSignalAt: number | null = null;
  ...
  getMetrics(): PersistenceQueueMetrics {
    return {
      ...
      ...(this.lastFlushMs !== null ? { lastFlushMs: this.lastFlushMs } : {}),
      ...
      duplicateSignalCount: this.duplicateSignalCount,
      lastDuplicateSignalAt: this.lastDuplicateSignalAt
    };
  }
  ...
  private flushQueue(): void {
    ...
    try {
      ...
      this.lastFlushMs = Date.now() - flushStartedAt;
      this.flushing = false;
      this.warnIfQueuePressure();
    }
  ...
  private persistSignal(input: SignalEventWriterInput): void {
    ...
    try {
      ...
      const signalResult = this.repository.createSignal({
  ...
  private recordDuplicateSignal(): void {
    this.duplicateSignalCount += 1;
    this.lastDuplicateSignalAt = Date.now();
  }
}
