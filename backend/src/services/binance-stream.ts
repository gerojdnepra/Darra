...
export class BinanceStreamManager {
  ...
  private buildMarketStreams(): string[] {
    ...
    const focusKlines = this.focusSymbols.map((symbol) => `${symbol.toLowerCase()}@kline_1m`);

    return ["!ticker@arr", "!markPrice@arr@1s", "!forceOrder@arr", ...focusAggTrades, ...focusKlines];
  }
  ...
  private handleMessage(route: "market" | "public", rawMessage: string): void {
    ...
    if (route === "market" && payload.stream.endsWith("@kline")) {
      const event = payload.data as KlineEvent;
      if (event.s && event.k && event.k.x) {
        this.callbacks.onKline(event);
      }
      return;
    }
